            ...
            <div className='w-[640px] rounded-xl py-5 px-6 shadow-shape bg-zinc-900 space-y-5'>
                ...
                <form onSubmit={createTrip} className='space-y-3'>
                    <div className='h-14 px-4 bg-zinc-950 border border-zinc-800 rounded-lg flex items-center gap-2'>
                        ...
                        <input type="text" name='name' placeholder='Seu nome completo' onChange={event => setOwnerName(event.target.value)}
                            className='bg-transparent placeholder:text-zinc-400 outline-none flex-1' />
                    </div>

                    <div className='h-14 px-4 bg-zinc-950 border border-zinc-800 rounded-lg flex items-center gap-2'>
                        <User className='text-zinc-400 size-5' />
                        <input type="email" name='email' placeholder='Seu e-mail pessoal' onChange={event => setOwnerEmail(event.target.value)}
                            className='bg-transparent placeholder:text-zinc-400 outline-none flex-1' />
                    </div>
